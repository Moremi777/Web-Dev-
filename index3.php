...
$bookstore = array("Bens Books","Carmens Chronicles");
$books = array
	(
...
$html = "<html><head></head><body><p id = 'xml'><stores>";
...
foreach($bookstore as $val)
{
	...
	{
		//$html.= "$tab$tab".htmlspecialchars("<subject>$info[0]</subject>")."<br>$tab$tab$tab$tab".htmlspecialchars("<author>".$info[2]."</author>")."<br>";
...
var htmlval = document.getElementById('xml').innerHTML;
 parser = new DOMParser();
	...
	var output = xmlDoc.getElementssyTagName(\"subject\")[0].childNodes[0].nodeValue;
